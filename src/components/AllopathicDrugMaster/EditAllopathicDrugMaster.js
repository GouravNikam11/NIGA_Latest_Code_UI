...
class EditAllopathicDrugMaster extends Component {
    constructor(props) {
        super(props)
        this.state = {
            allopathicDrugName: '',
    ...
    async componentDidMount() {
        var Id = this.props.match.params.allopathicDrugId;
        this.editallopathicDrug(Id);
        this.getdrugGroup();
    }
    ...
    deleteSideEffect = (index, string,id) => {
        debugger
        if (string === 'seriousSideEffectName') {
            ...
            if (index !== -1) {
                ...
                CommonServices.postData({seriousSideEffectId:id}, `/SeriousSideEffect/DeleteSeriousSideEffect`).then((res) => {
                });
            }
        }
            ...
            if (index !== -1) {
                ...
                CommonServices.postData({otherSideEffectId:id}, `/OtherSideEffect/DeleteOtherSideEffect`).then((res) => {
                });
            }
        }
        else {
            ...
            if (index !== -1) {
                ...
                CommonServices.postData({adverseReactionId:id}, `/AdverseReaction/DeleteAdverseReaction`).then((res) => {
                });
            }
        }
    }

    editallopathicDrug(Id) {
        debugger;
        if (Id !== undefined) {
            CommonServices.getDataById(Id, `/AllopathicDrug`).then((res) => {
                this.setState({
...

                                                {
                                                    this.state.seriousSideEffectModelList.map((s, index) => {
                                                        return <tr key={index}>
                                                            ...
                                                            <td className='lcol'>
                                                                <Button style={{ marginLeft: 8 }} variant="danger" color="danger" onClick={() => this.deleteSideEffect(index, 'seriousSideEffectName',s.seriousSideEffectId)} ><i className="fa fa-trash"></i></Button>
                                                            </td>
                                                        </tr>
                                                    })
                                                }
                                            ...
                                            <tbody>
                                                {
                                                    this.state.otherSideEffectModelList.map((s, index) => {
                                                        return <tr key={index}>
                                                            ...
                                                            <td className='lcol'>
                                                                <Button style={{ marginLeft: 8 }} variant="danger" color="danger" onClick={() => this.deleteSideEffect(index, 'otherSideEffectName',s.otherSideEffectId)} ><i className="fa fa-trash"></i></Button>
                                                            </td>
                                                        </tr>
                                                    })
                                                }
                                            ...
                                            <tbody>
                                                {
                                                    this.state.adverseReactionModelList.map((s, index) => {
                                                        return <tr key={index}>
                                                            ...
                                                            <td className='lcol'>
                                                                <Button style={{ marginLeft: 8 }} variant="danger" color="danger" onClick={() => this.deleteSideEffect(index, 'adverseReactionName',s.adverseReactionId)} ><i className="fa fa-trash"></i></Button>
                                                            </td>
                                                        </tr>
                                                    })
                                                }
    ...
    submitForm() {
        debugger
        var obj = {
            "allopathicDrugId": this.state.allopathicDrugId,
...
export default connect(null, { enqueueSnackbarAction, closeSnackbar })(EditAllopathicDrugMaster)
